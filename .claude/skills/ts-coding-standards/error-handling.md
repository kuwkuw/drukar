...
> **⚠ Drukar divergence**: this repo currently **throws at provider/IO boundaries and catches
> at the agent loop and routes** (see `apps/api/src/agent/loop.ts`) — consistent with this
> document's own table row "truly exceptional situations: throw + try-catch". Result types /
> neverthrow are an accepted future direction, not the present convention; match surrounding
> code rather than mixing styles within a module.
