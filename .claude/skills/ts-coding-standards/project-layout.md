...
> **⚠ Drukar divergence**: this document describes an aspirational Bun/Clean-Architecture
> layout. **This repo is a pnpm workspace with a deliberate 3-package layout**
> (`packages/shared` → `apps/api` / `apps/web`) and `index.ts` entry points — see CLAUDE.md,
> which wins wherever the two disagree. Use this file for its principles (dependency direction,
> kebab-case naming, co-located tests), not its structure.
